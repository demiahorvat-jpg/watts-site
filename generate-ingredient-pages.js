...
function generateIngredientPage(ingredient) {
  const slug = slugify(ingredient.name);

  // Capitalize category for display
  const categoryDisplay = ingredient.category.charAt(0).toUpperCase() + ingredient.category.slice(1);

  // Badge class based on position
  const badgeClass = {
    'good': 'badge-good',
    'neutral': 'badge-neutral',
    'caution': 'badge-caution',
    'avoid': 'badge-avoid'
  }[ingredient.wattsPosition] || 'badge-neutral';

  const badgeLabel = ingredient.wattsPosition.charAt(0).toUpperCase() + ingredient.wattsPosition.slice(1);

  // Nutrition value display text
  const nutritionText = {
    'high': 'High nutritional value',
    'moderate': 'Moderate nutritional value',
    'low': 'Low nutritional value',
    'none': 'No nutritional value'
...
<head>
    ...
    <script type="application/ld+json">
    {
      ...
      "@type": "Article",
      "headline": "${ingredient.name} in Dog Food: What It Is & Why It's Used",
  ...
  <main>
    ...
    <div class="ingredient-hero">
      ...
      <div class="ingredient-meta">
        ...
        <div class="nutrition-value">${nutritionText}</div>
      </div>
    </div>
...
${ingredient.whyUsed.map(reason => `      <li>${reason}</li>`).join('\n')}
    ...
    <div class="quality-note-box">
      <div class="quality-note-label">Quality Considerations</div>
    ...
    <div class="cta-section">
      ...
      <p>Paste your dog food ingredient list to see what's really in it.</p>
...
// Find related ingredients (same category, limit to 4)
function findRelatedIngredients(ingredient, allIngredients) {
...
function generateAllPages() {
  ...
  const baseDir = './ingredient-analyzer/ingredients';
  ...
  const slugMap = {}; // Track slugs for the index page

  ingredients.forEach(ingredient => {
    const { slug, html } = generateIngredientPage(ingredient);
    const dir = path.join(baseDir, slug);

    // Create ingredient directory
  ...
  console.log(`\n✅ Successfully generated ${generatedCount} ingredient pages!`);

  // Save slug map for later use
  fs.writeFileSync(
    './ingredient-analyzer/ingredient-slugs.json',
    JSON.stringify(slugMap, null, 2)
  );

  console.log('✅ Saved slug map to ingredient-slugs.json');
}

// Run the generator
console.log('🚀 Generating ingredient pages...\n');
generateAllPages();
